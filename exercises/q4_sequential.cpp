...
int find_primary_index(vector<long long> &primary_indexes, long long search_value)
{
	...
	while(!search_value_is_lesser_or_equals_element && index < SIZE_OF_PRIMARY_INDEXES)
	{
	...
	if(index > 1)
	{
		...
		else
		{
			...
			// The -2 is to get the previous index with index correction (which last is SIZE-1)
...
int sequential_search(vector<long long> &sorted_elements, long long search_value, int initial_index)
{
	...
	for(index = initial_index; index < SIZE_OF_SORTED_ELEMENTS; ++index)
	{
		...
		if(sorted_elements[index] > search_value)
		{
			// The element is bigger than the value, so the value could not be found
			return NOT_FOUND;
		}
	}
	// The element was not found (uncanny behavior)
	return NOT_FOUND;
}
